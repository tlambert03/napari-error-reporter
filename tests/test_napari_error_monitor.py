...
from napari_error_monitor import ask_opt_in, get_sample_event, install_error_monitor


@pytest.fixture(autouse=True)
def mock_settings(tmp_path, monkeypatch):
    assert str(napari_error_monitor.settings_path()).endswith(".json")
    monkeypatch.setattr(
        napari_error_monitor, "settings_path", lambda: tmp_path / "test.json"
    )


def test_widget(qtbot):
    wdg = napari_error_monitor.OptInWidget()
    qtbot.addWidget(wdg)
    wdg._set_no()
    assert wdg._no
...
def test_opt_in():
    ...
    assert not napari_error_monitor.settings_path().exists()

    mock = MagicMock(return_value=None)
    setattr(napari_error_monitor.OptInWidget, "exec", mock)

    assert mock.call_count == 0
    assert ask_opt_in()["enabled"] is None
    assert mock.call_count == 1

    assert napari_error_monitor.settings_path().exists()
